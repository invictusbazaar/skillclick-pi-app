...
import { 
  LogOut, ChevronDown, User as UserIcon
} from "lucide-react"
import { 
...
function NavbarContent() {
  ...
  const dropdownItemClass = "w-full cursor-pointer text-gray-700 font-bold py-3 text-sm flex items-center gap-3 transition-all duration-500 ease-out rounded-md outline-none hover:bg-purple-100 hover:text-purple-700 focus:bg-purple-100 focus:text-purple-700 active:bg-purple-100 active:text-purple-700 active:scale-90";
  ...
  return (
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-[100] shadow-sm flex flex-col font-sans">
      ...
      <div className="container mx-auto px-4 h-20 flex items-center justify-between relative">
        <div className={`flex-shrink-0 absolute left-0 top-1/2 -translate-y-1/2 z-[40] -ml-[156px] md:-ml-[220px] mt-2 md:mt-[11px]`}>
           <Link href="/"> 
        ...
        <div className="hidden md:flex items-center gap-4 ml-auto z-[80]">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-white border-gray-100 shadow-lg p-1 z-[200]">
              {Object.entries(languages).map(([key, { label, flag }]) => (
                <DropdownMenuItem key={key} onClick={() => setLanguage(key)} className={dropdownItemClass}>
          ...
          {/* --- GLAVNO DUGME: POST A SERVICE --- */}
          {/* Jedino dugme koje nam treba. Pametno vodi gde treba. */}
          <Link href={user ? "/create" : "/register?redirect=/create"}>
            <Button 
                variant="ghost"
                className={`
                    text-gray-600 font-bold text-sm h-10 px-4
                    hover:text-purple-600 hover:bg-purple-50 
                    ${clickEffect} ${noFocusRing}
                `}
            >
                Post a Service
            </Button>
          ...
          {/* UKLONJENO DUGME "BECOME A SELLER" - VIŠAK JE */}
          ...
          {user ? (
            <div className="flex items-center gap-4 border-l border-gray-200 pl-4">
                ...
                <Button onClick={logout} variant="ghost" className={`text-gray-500 hover:text-red-600 hover:bg-red-50 p-2 h-10 w-10 rounded-full ${clickEffect} ${noFocusRing}`}><LogOut className="w-5 h-5" /></Button>
            </div>
          ) : (
