...
import { 
  ArrowLeft, CreditCard, Star, Clock, RefreshCcw, 
  MessageCircle, Send, User, Wrench, Car, Code, 
  PenTool, Palette, Video, Briefcase, Coffee
} from 'lucide-react';
...
export default function ServiceDetail() {
  ...
  const [piReady, setPiReady] = useState(false); // Da znamo da li je Pi spreman

  // --- BOJE I IKONE ---
  const getGradient = (id: string) => {
    if (!id) return "from-indigo-500 to-purple-600";
    const sum = id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const gradients = ["from-fuchsia-600 to-pink-600", "from-violet-600 to-indigo-600", "from-blue-600 to-cyan-500", "from-emerald-500 to-teal-600", "from-orange-500 to-red-600", "from-purple-600 to-blue-600"];
  ...
  const getSmartIcon = (svc: any) => {
    ...
    if (cat.includes('business')) return <Briefcase className={iconClass} />;
  ...
  // --- POPRAVLJENA PI INICIJALIZACIJA ---
  useEffect(() => {
      const initPi = async () => {
          if (!window.Pi) {
              console.log("Pi SDK još nije učitan, čekam...");
              return;
          }
          try { 
              await window.Pi.init({ version: "2.0", sandbox: false }); // Promeni na sandbox: true ako testiraš
      ...
      // Proveravamo svakih 500ms da li je skripta stigla
  ...
  const handlePayment = async () => {
    ...
    alert(`Započinjem plaćanje: ${amountNum} Pi`); // Debug za tebe
    ...
    try {
      ...
      const callbacks = {
        ...
        onReadyForServerCompletion: async (paymentId: string, txid: string) => {
          // alert("Završavam porudžbinu...");
          await fetch('/api/payments/complete', {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
             body: JSON.stringify({ paymentId, txid, serviceId: service.id }),
          });
          alert("✅ Plaćanje uspešno! Čestitamo.");
          router.push('/'); // Vraćamo na početnu
        },
        onCancel: () => alert("Plaćanje otkazano."),
        onError: (e: any) => alert(`Greška pri plaćanju: ${e.message || e}`),
      };
      ...
      await window.Pi.createPayment(paymentData, callbacks);
        ...
        console.error(e);
        alert("Nije uspelo pokretanje plaćanja: " + e.message);
    }
  };

  const handleContact = () => {
      alert("Kontakt forma uskoro!");
  };

  if (!service) return <div className="p-20 text-center text-purple-600 font-bold">Učitavanje oglasa...</div>;
