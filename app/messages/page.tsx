...
function ChatInterface() {
  ...
  const [chatHistory, setChatHistory] = useState([
    { id: 1, text: "...", sender: "system", time: "10:00", type: "welcome" } 
  ])

  // 1. Učitavanje Inbox liste (ako nismo u konkretnom čatu)
  useEffect(() => {
  ...
  // 2. Prevođenje sistemskih poruka u čatu
  useEffect(() => {
    if (sellerName) {
  ...
  const handleSend = async () => {
    ...
    const optimisticMsg = { id: Date.now(), text: contentToSend, sender: "me", time: "Now", type: "user" };
          ...
          <div className="max-w-2xl mx-auto bg-white min-h-screen md:shadow-2xl md:border-x md:border-gray-100">
            ...
            <div className="p-4">
                <div className="relative group">
                    ...
                    <input type="text" placeholder="Search..." className="w-full bg-gray-100 border-none rounded-xl py-3 pl-10 pr-4 text-sm outline-none" />
                </div>
            </div>
            {loadingInbox ? (
              ...
              <div className="flex flex-col">
                {conversations.map((conv) => (
                  <div key={conv.username} onClick={() => router.push(`/messages?seller=${conv.username}`)} className="px-4 py-4 flex items-center gap-4 hover:bg-purple-50 cursor-pointer transition-all border-b border-gray-50 group">
                    <div className="w-14 h-14 bg-gradient-to-br from-purple-500 to-purple-700 text-white rounded-2xl flex items-center justify-center font-bold text-xl shadow-md">{conv.username[0].toUpperCase()}</div>
    ...
    <div className="h-[100dvh] md:min-h-screen w-full bg-white md:bg-gray-100 flex flex-col md:items-center md:justify-center font-sans overflow-hidden">
      <div className="w-full md:max-w-2xl bg-white md:rounded-2xl md:shadow-xl md:border md:border-gray-200 h-full md:h-[85vh] flex flex-col relative">
          ...
          {chatHistory.map((msg) => (
              <div key={msg.id} className={`flex w-full ${msg.sender === "me" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[85%] md:max-w-[70%] p-3.5 rounded-2xl shadow-sm text-sm md:text-base leading-relaxed break-words
                      ${msg.sender === "me" ? "bg-purple-600 text-white rounded-br-none" : msg.sender === "system" ? "bg-transparent text-gray-400 text-center text-xs w-full shadow-none my-2" : "bg-white border border-gray-200 text-gray-700 rounded-bl-none"}`}>
                      <p>{msg.text}</p>
        ...
        <div className="bg-white border-t border-gray-100 p-3 md:p-4 shrink-0 w-full pb-safe">
            <div className="flex gap-2 items-center bg-gray-50 border border-gray-200 rounded-2xl px-2 py-2 focus-within:ring-2 focus-within:ring-purple-100 transition-all">
                ...
                <Button onClick={handleSend} disabled={isSending} size="icon" className="shrink-0 rounded-xl bg-purple-600 hover:bg-purple-700 text-white disabled:opacity-50">
