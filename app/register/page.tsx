...
import { useRouter, useSearchParams } from 'next/navigation'; // Dodali smo useSearchParams
...
export default function LoginPage() {
  ...
  const searchParams = useSearchParams(); // Ovde hvatamo instrukcije iz linka
  const { login } = useAuth();
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Simulacija prijave
    setTimeout(() => {
      // 1. Proveravamo da li imamo sačuvane podatke
      ...
      // 2. Logujemo korisnika
      login(displayName, email);

      // 3. PAMETNO PREUSMERAVANJE (Ovo je ključna izmena)
      // Proveravamo da li postoji "redirect" u linku (npr. ?redirect=/create)
      const redirectUrl = searchParams.get('redirect');

      if (redirectUrl) {
        // Ako postoji instrukcija (npr. idi pravi oglas), vodi ga tamo
        router.push(redirectUrl);
      } else {
        // Ako nema instrukcije, vodi ga na početnu kao i obično
        router.push('/');
      }
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl shadow-purple-900/10 border border-white overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="p-8 pt-0">
          ...
          <div className="mt-8 text-center text-sm text-gray-500 border-t border-gray-100 pt-6">
            Nemaš nalog?{' '}
            {/* I ovde moramo da prosledimo redirect ako postoji, da se ne izgubi ako korisnik klikne na registraciju iz logina */}
            <Link 
